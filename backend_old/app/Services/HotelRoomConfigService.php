...
class HotelRoomConfigService implements HotelRoomConfigServiceInterface
{
    ...
    public function crear(int $hotelId, array $data): array
    {
        ...
        $total = collect($this->repo->allByHotel($hotelId))->sum('cantidad');
        if ($total + $data['cantidad'] > $hotel->max_habitaciones) {
    ...
    {
        if (!$this->repo->delete($id)) throw new NotFoundHttpException('Configuración no encontrada');
    }
}
